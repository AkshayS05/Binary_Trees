...
private static boolean findNode(BinaryTree<Integer>root,int target) {
//        if root is null, there is no node to find the target, return false;
    if (root == null) {
        return false;
    }
//    check if the root is the one we are looking for
    if (root.data == target) {
        return true;
    }
    boolean isPresent=false;
//    explore the root's left
    if (root.left != null) {
        isPresent=findNode(root.left, target);
    }
    if(!isPresent) {
//    explore root's right
        isPresent = findNode(root.right, target);
    }
    return isPresent;
}
    public static void main(String[] args) {
        ...
        int target=7;
        System.out.println(findNode(root,target));
    }
}
